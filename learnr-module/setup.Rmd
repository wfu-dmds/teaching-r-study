...
tutorial_event_recorder <- function(username) {
  ...
  recorder <- function(tutorial_id, tutorial_version, user_id,
                       event, data) {
    if (rdrop2::drop_exists(glue::glue("teaching-r-study/data_{username}.rds"))) {
      rdrop2::drop_download(path = glue::glue("teaching-r-study/data_{username}.rds"),
    ...
    rdrop2::drop_upload(file = glue::glue("data_{username}.rds"),
                path = glue::glue("teaching-r-study"))
  }
  recorder
}
...
if (!base::is.null(query[["user_id"]])) {
  user_id <- stringr::str_extract(string = query[["user_id"]], 
                                  pattern = "([^/]+)")
